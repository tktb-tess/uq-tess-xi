<script lang="ts">
  import { safeFetchJson } from '$lib/modules/util';
  ...
  let resultAsy = $state(fetchTodayWord());
...
>
  {#await resultAsy then result}
    {#if result.isOk()}
      ...
      <p class="self-end me-3"><ExtLink href={value.dicUrl} class="">ZpDIC Online</ExtLink></p>
    {:else}
