 ...
 * @param length ビット長
 * @param fixed true: 固定長, false (デフォルト値): length_ ビット以下の可変ビット長
 * @returns length ビットの乱数
*/
export const getRandBigInt = (length: number, fixed = false) => {
...
/** 遅延評価関数化する */
export const lazify = <ArgT extends unknown[], RetT>(func: (...args: ArgT) => RetT) => (...args: ArgT) => () => func(...args);
